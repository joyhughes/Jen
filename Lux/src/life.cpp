...
{ 
    ...
    if( *blur_method == BB_CUSTOM ) {
        ...
        if( context.s.functions.contains( custom_picker ) ) {
            auto& pickers = std::get< std::shared_ptr< custom_blur_picker > >( std::get< any_fn< int > >( context.s.functions[ custom_picker ] ).any_int_fn );
        ...
        case BB_CUSTOM: {
            ...
            if( *random_copy ) {
                ...
                if( size > 0 ) {
                    ...
                    g += random_add; g /= size;
                    b += random_add; b /= size;
                    result[ 0 ] = 0xff000000 | ( r << 16 ) | ( g << 8 ) | b;
                }
                else result[ 0 ] = MM;
            }         
        }
