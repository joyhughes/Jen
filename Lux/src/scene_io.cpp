...
void scene_reader::read_image(const json &j) {
    ...
    if (type == "uimage") {
        ...
        s.buffers[name] = img;
        std::cout << "scene_reader: Created uimage buffer_pair for name='" << name
                << "' from file='" << filename << "'. Initialization/Mipmapping should follow." << std::endl;
    }
...
void scene_reader::read_function(const json &j) {
    ...
    if (j.contains("name")) {
        ...
        ERROR("Function definition missing required 'name' field.\n");
        return;
    }
    if (j.contains("type")) {
        j["type"].get_to(type);
    } else {
        ERROR("Function '" + name + "' missing required 'type' field.\n");
        return;
    }

    DEBUG("scene_reader::read_function - name: '" + name + "' type: '" + type + "'");

    // --- Handle bool_function_condition explicitly ---
    if (type == "bool_function_condition") {
        DEBUG("  Processing type: bool_function_condition");
        std::string source_name = "";
        if (j.contains("source_function_name")) {
            j["source_function_name"].get_to(source_name);
        } else {
            ERROR("bool_function_condition '" + name + "' is missing required 'source_function_name' field.");
            return;
        }
        std::shared_ptr<bool_function_condition> fn_obj_ptr(new bool_function_condition(source_name));
        condition_fn callable_fn = [fn_obj_ptr](element_context &context) -> bool {
            return (*fn_obj_ptr)(context);
        };
        any_condition_fn_ptr variant_ptr = fn_obj_ptr;
        any_condition_fn cond_func_wrapper(variant_ptr, callable_fn, name);
        s.functions[name] = cond_func_wrapper; // Store as any_function variant
        DEBUG("!!! Stored function '" + name + "' as any_condition_fn in map !!!");
    }
    if (type == "menu_string") {
         DEBUG("  Processing type: menu_string (Explicit Handler)");
         // Create the specific object pointer
         auto fn_ptr = std::make_shared<menu_string>(); // menu_string is typedef for menu

         // Read properties specific to menu_string/menu
         if(j.contains("label")) read(fn_ptr->label, j["label"]);
         if(j.contains("description")) read(fn_ptr->description, j["description"]);
         if(j.contains("default_choice")) read(fn_ptr->default_choice, j["default_choice"]);
         if(j.contains("tool")) read(fn_ptr->tool, j["tool"]);
         if(j.contains("affects_widget_groups")) read(fn_ptr->affects_widget_groups, j["affects_widget_groups"]);
         if(j.contains("rerender")) read(fn_ptr->rerender, j["rerender"]);
         fn_ptr->choice = fn_ptr->default_choice; // Set initial choice
         if (j.contains("items")) {
             for (const auto& item_json : j["items"]) { // Use const auto&
                std::string item;
                item_json.get_to(item);
                fn_ptr->add_item(item);
             }
         }

         // Create the std::function wrapper (type string_fn)
         // *** Assumes menu::operator()(std::string&, element_context&) exists ***
         string_fn callable_fn = [fn_ptr](std::string& v, element_context& ctx){
             return (*fn_ptr)(v,ctx);
         };

         // Create the variant pointer for the specific any_fn type
         // *** Requires std::shared_ptr<menu_string> in any_string_fn_ptr variant ***
         any_string_fn_ptr variant_ptr = fn_ptr;

         // Construct the specific any_fn<string> wrapper
         // *** Assumes any_fn<string> constructor takes (variant, function, name) ***
         any_fn<string> func_wrapper(variant_ptr, callable_fn, name);

         // Store the wrapper (which is implicitly convertible to any_function)
         s.functions[name] = func_wrapper;
         DEBUG("  Stored menu_string '" + name + "' as any_fn<string> in functions map.");
    }

        DEBUG("  Processing function type using macros...");
...
void scene_reader::read_effect(const json &j) {
    ...
    // *** NEW FILTER HANDLING BLOCK - Starts here ***
    if (type == "filter") {
        DEBUG("Handling filter effect: '" + name + "'");

        // Create the filter object directly
        std::shared_ptr<filter> the_filter = std::make_shared<filter>();

        bool dependencies_ok = true; // Flag to track if everything is found

        // Add conditions listed in JSON
        if (j.contains("conditions")) {
            DEBUG("  Reading conditions for filter '" + name + "'...");
            for (const auto &cond_name_json: j["conditions"]) {
                std::string cond_name;
                cond_name_json.get_to(cond_name);
                if (s.functions.count(cond_name) && std::holds_alternative<any_condition_fn>(s.functions[cond_name])) {
                    the_filter->add_condition(std::get<any_condition_fn>(s.functions[cond_name]));
                    DEBUG("    Added condition: '" + cond_name + "'");
                } else {
                    ERROR("ERROR: Condition '" + cond_name + "' for filter '" + name +
                        "' not found in s.functions or is not any_condition_fn.");
                    dependencies_ok = false;
                }
            }
        }

        // Add functions listed in JSON
        if (j.contains("functions")) {
            DEBUG("  Reading functions for filter '" + name + "'...");
            for (const auto &func_name_json: j["functions"]) {
                std::string func_name;
                func_name_json.get_to(func_name);
                if (s.functions.count(func_name) && std::holds_alternative<any_gen_fn>(s.functions[func_name])) {
                    the_filter->add_function(std::get<any_gen_fn>(s.functions[func_name]));
                    DEBUG("    Added function: '" + func_name + "'");
                } else {
                    ERROR("ERROR: Function '" + func_name + "' for filter '" + name +
                        "' not found in s.functions or is not any_gen_fn.");
                    dependencies_ok = false;
                }
            }
        }

        // Store the effect ONLY if dependencies were okay
        if (dependencies_ok) {
            // Create the std::function wrapper lambda
            any_effect_fn::effect_fn callable_fn =
                    [the_filter](any_buffer_pair_ptr &buf, element_context &context) {
                (*the_filter)(buf, context); // Call filter::operator()
            };

            // Create the variant pointer
            any_effect_fn_ptr variant_ptr = the_filter; // Assign shared_ptr to variant

            // Construct and store the effect wrapper
            s.effects[name] = any_effect_fn(variant_ptr, callable_fn, name);
            DEBUG("  Created and stored filter '" + name + "' in effects map.");
        } else {
            std::cerr << "  ERROR: Failed to fully configure filter '" + name +
                    "' due to missing dependencies. Not storing in effects map." << std::endl;
            // Decide if you want to halt completely:
            // ERROR("Failed to configure filter '" + name + "'");
        }
    } else {
