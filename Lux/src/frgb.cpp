...
float r( const frgb &c )       { return c.R; }
float g( const frgb &c )       { return c.G; }
float b( const frgb &c )       { return c.B; }
...
// set component
void setr( frgb &c, const float& r )   { c.R = r; }
void setg( frgb &c, const float& g )   { c.G = g; }
void setb( frgb &c, const float& b )   { c.B = b; }
...
void apply_mask( const frgb &base, const frgb &mask, frgb &result ) {
    ...
    result.B = result.B * mask.B + base.B * ( 1.0 - mask.B);*/
}
