...
(async () => {
  try {
    console.log('[WASM] Loading lux.js with integrated camera optimizations...');
    ...
    factory().then((instance) => {
      ...
      const allFunctions = Object.keys(window.module).filter(key => typeof window.module[key] === 'function');
      ...
      if (missingUltraFunctions.length === 0 && missingBasicFunctions.length === 0) {
        console.log('[WASM] ✅ Module initialized with ULTRA camera optimizations');
        ...
        if (missingUltraFunctions.length > 0) {
          console.warn('[WASM] Missing ultra camera functions:', missingUltraFunctions);
        }
        if (missingBasicFunctions.length > 0) {
          console.warn('[WASM] Missing basic functions:', missingBasicFunctions);
        }
      ...
      console.log('[WASM] Ultra camera ready:', window.module.ultraCameraReady);
