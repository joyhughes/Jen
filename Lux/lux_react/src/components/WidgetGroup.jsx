...
function WidgetGroup({
  ...
  const getBreakpointColumns = () => {
    ...
    // More aggressively create columns - minimum width reduced to 140px
    ...
    console.log(
      `WidgetGroup: Available width ${availableWidth}px, columns: ${maxColumns}`
    );
  ...
  const createWidgetElement = (name) => {
    ...
    switch (widget.type) {
      case "menu_int":
        return (
          <Box key={widget.name} sx={{ mb: 0.5, width: "100%" }}>
      ...
      case "funk_factor_picker":
        return (
          <Box key={widget.name} sx={{ width: "100%", mb: 0.5 }}>
            <Box sx={{ mt: 0.5 }}>
      ...
      case "custom_blur_picker":
        ...
        return (
          <Box key={widget.name} sx={{ width: "100%", mb: 0.5 }}>
            <Box sx={{ mb: 0.5 }}>{labelElement}</Box>
  ...
  return widgetElements.map((element, index) => (
