...
### Lux Vitae - Scripted Compositing, L-Systems, and Animation - C version
December 2021 <br>
...
### Element
An element consists of any image type mapped to the range -1.0 to 1.0 in x and y and optionally a mask of the same size and type as the image. It has a position, size, and orientation within the scene that can be controlled by 
...
### Scene
A scene contains a complete description of the result image, including each element, function tree, and cluster, each of which might be used multiple times within the scene. It can also include stencils, images (which may or may not be displayed) which can influence the functions that generate clusters, for instance by seeking or avoiding certain colors or moving towards brighter or darker areas, or changing element properties such as brightness, color, size, or orientation depending on the stencil pixel value. The scene code (part of Lux Vitae) includes a function to render the scene. Scenes are loaded from scene files, which can be written by hand or generated by Lux.

### Generative Space
First implemented in Still Life, a generative space is a set of rules for generating scene files. The generative space includes a list of possible elements, function trees, clusters, and effects, and the relative probability that each will appear in a generated scene. 

## Images

### Implementation
An image is defined as an evenly spaced grid of values of given x and y dimensions
