...
### Func_tree
A function tree is a method for representing generalized functions based on a tree structure of nodes each of which performs a simple mathematical function. Nodes can have one, two, or three arguments, and can process take inputs in a mix of boolean, integer, floating point, vector, color, or array values. Each node can be accessed by its name. Function trees are stored in .jen files, which can be written by hand or generated by Niffer. The functions are used for image effects such as Warp, Melt, and Hyperspace, and for generating elements in clusters. In the future they may be used for artificial evolution. 

### Effect
Image effects include Warp, Melt, Hyperspace, and Life (described above), but additional effects may be added. Each effect is driven by a func_tree - except for Life, which is driven by a description of its rule set. Effects can be applied to a standalone image or to elements within a scene rendered by Lux.

### Element
An element consists of an FImage mapped to the range -1.0 to 1.0 in x and y, optionally an FImage mask of the same size as the image, and also optionally an image effect such as Warp, Melt, Hyperspace, or Life, driven by a func_tree or Life rules. The Lux renderer will "splat" an element onto a background image. An element can serve as a sprite within an animation, or a layer within a scene. An element description is stored as part of a scene file.

### Cluster
A cluster is a grouping of elements. A func_tree determines the position, orientation, scale, and color of each element, and can drive the element's image effect. Some arrangements for clusters include circles, grids, random arrangements, and along the flow lines of a vector field. Clusters can branch, and the branches can branch, producing L-systems that can resemble natural phenomena. The aurora simulation pictured above is produced by an L-system with three levels. A cluster description is stored as part of a scene file.

### Scene
A scene contains a complete description of the result image, including each element, function tree, and cluster, each of which might be used multiple times within the scene. It can also include stencils, images (which may or may not be displayed) which can influence the functions that generate clusters, for instance by seeking or avoiding certain colors or moving towards brighter or darker areas, or changing element properties such as brightness, color, size, or orientation depending on the stencil pixel value. The scene code (part of Lux Vitae) includes a function to render the scene. Scenes are loaded from scene files, which can be written by hand or generated by Niffer.

### Generative Space
First implemented in Still Life, a generative space is a set of rules for generating scene files which will be re-implemented as part of Niffer. The generative space includes a list of possible elements, function trees, clusters, and effects, and the relative probability that each will appear in a generated scene. Some parameters of the function trees can vary randomly over a range - the user will use "range sliders" with two knobs to specify these ranges.
